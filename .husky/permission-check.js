  ...
  relevantFiles.forEach(file => {
    ...
    if (file.includes('page.tsx') || file.includes('ClientPage.tsx')) {
      ...
      if (!content.includes('withPermission(')) {
        warnings.push(
          `⚠️  ${file} podría necesitar protección con withPermission`
        );
      }
      ...
      if (!content.includes('@permissions')) {
        warnings.push(
          `⚠️  ${file} no tiene documentación de permisos (@permissions)`
        );
      }
      ...
      if (routeMatch && routeMatch[1]) {
        ...
        if (
          !permissionsContent.includes(`'${route}'`) &&
          !permissionsContent.includes(`"${route}"`)
        ) {
          errors.push(
            `❌  La ruta '${route}' en ${file} no está definida en permissions.ts`
          );
        }
      }
    }
...
if (warnings.length > 0) {
  ...
  readline.question(
    chalk.yellow('Hay advertencias. ¿Continuar con el commit? (s/n): '),
    answer => {
      readline.close();

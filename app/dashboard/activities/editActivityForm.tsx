...
export default function EditActivityForm({ activity, onSuccess }: { activity: any, onSuccess?: () => void }) {
  const [name, setName] = useState(activity.name || "");
  const [requiredDriverLicense, setRequiredDriverLicense] = useState(activity.requiredDriverLicense || "");
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow mb-8">
      ...
      <div className="mb-4">
        ...
        <input type="text" value={requiredDriverLicense} onChange={e => setRequiredDriverLicense(e.target.value)} className="w-full p-2 border rounded" placeholder="Ej: Clase B" />
      </div>
      <div className="mb-4">
        <label className="block mb-1 font-medium">Imagen referencial (opcional)</label>
        <input type="file" accept="image/*" onChange={e => setImageFile(e.target.files?.[0] || null)} className="w-full" />
      </div>
      <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded" disabled={loading}>{loading ? "Guardando..." : "Guardar cambios"}</button>

...
const FormRegister = ({
  ...
  const FormInputs = [
    ...
    },
    {
      ...
      required: true,
      small: 'Como te llamaremos en la plataforma',
    },
    {
      name: 'email',
    ...
    },
    {
      name: 'phoneNumber',
      label: 'Teléfono',
      placeholder: 'ej:52 2 23 24 25 ó 9 9876 1234',
      required: true,
      icon: Phone,
      small: 'Formato: +56 xxxxxxxxx ',
    },
  ];
  const roleOptions = [
    ...
    startTransition(async () => {
      try {
        ...
        Object.keys(valuesForValidation).forEach(key => {
          const value =
            valuesForValidation[key as keyof typeof valuesForValidation];
        ...
        if (isEditing && initialData?.id) {
          ...
          if (!valuesForValidation.password) {
            form.setError('password', {
              type: 'manual',
              message:
                'La contraseña es obligatoria para registrar un nuevo usuario',
            });
            setError(
              'La contraseña es obligatoria para registrar un nuevo usuario'
            );
        ...
        if (actionResponse.error) {
          ...
          if (
            actionResponse.validationErrors &&
            actionResponse.validationErrors.length > 0
          ) {
            console.error(
            ...
            setError(
              `Datos inválidos: ${actionResponse.validationErrors.map(e => `${e.path}: ${e.message}`).join(', ')}`
            );
                      ...
                      <div className="relative">
                        ...
                        <Image
                          ...
                          onError={() => {
                            ...
                            console.error(
                              'Error al cargar la imagen de Cloudinary:',
                              selectedImage
                            );
